...
/internal
├─ /internal/login
├─ /internal/mypage/password-change
├─ /internal/dashboard
├─ /internal/notices
...
| `/internal/mypage/password-change`     | 비밀번호 변경    | 초기/주기 비밀번호 변경       | 예        | TEAM_MEMBER 이상 (상태 게이트) | 현재 계정 상태, 비밀번호 정책             | 현재/신규 비밀번호 제출  | MVP      |
| `/internal/dashboard`                  | 대시보드         | 내부 서비스 홈                | 예        | TEAM_MEMBER 이상 | 요약 통계, 최근 공지, 결재 대기, 차단 건수 | 이동                     | MVP      |
| `/internal/notices`                    | 사내 공지 목록   | 공지 조회                     | 예        | TEAM_MEMBER 이상 | 공지 목록                                  | 상세 이동                | MVP      |
| `/internal/notices/:noticeId`          | 사내 공지 상세   | 공지 상세 조회                | 예        | TEAM_MEMBER 이상 | 공지 상세                                  | 수정/삭제(권한 시)       | MVP      |
| `/internal/employees`                  | 사원 디렉토리    | 조직 구성원 조회              | 예        | TEAM_MEMBER 이상 | 사원 목록                                  | 검색, 상세 이동          | MVP      |
| `/internal/employees/:employeeId`      | 사원 상세        | 사원 정보 조회                | 예        | TEAM_MEMBER 이상 | 사원 상세                                  | 조회                     | 추천     |
| `/internal/approvals`                  | 전자결재 목록    | 결재 문서 조회                | 예        | TEAM_MEMBER 이상 | 결재 목록                                  | 상세 이동, 승인/반려     | MVP      |
| `/internal/approvals/new`              | 전자결재 작성    | 결재 문서 생성                | 예        | TEAM_MEMBER 이상 | 결재 입력값                                | 제출                     | MVP      |
| `/internal/approvals/:approvalId`      | 전자결재 상세    | 결재 상세 및 처리             | 예        | TEAM_MEMBER 이상 | 결재 상세                                  | 승인, 반려               | MVP      |
| `/internal/applicants`                 | 지원자 목록      | 지원서 검토 목록              | 예        | TEAM_MEMBER 이상 (HR 또는 ADMIN, EXECUTIVE 읽기 전용 선택) | 지원자 목록                                | 상세 이동, 상태 필터     | MVP      |
| `/internal/applicants/:applicationId`  | 지원자 상세      | 지원서 및 첨부 검토           | 예        | TEAM_MEMBER 이상 (HR 또는 ADMIN, EXECUTIVE 읽기 전용 선택) | 지원자 상세, 파일 상태                     | 상태 변경, 메모 작성     | MVP      |
| `/internal/support-tickets`            | 문의 목록        | 고객 문의 처리                | 예        | TEAM_MEMBER 이상 (MAINTENANCE 또는 ADMIN, EXECUTIVE 읽기 전용 선택) | 문의 목록                                  | 상세 이동, 상태 필터     | MVP      |
| `/internal/support-tickets/:ticketId`  | 문의 상세        | 문의 내용 및 첨부 검토        | 예        | TEAM_MEMBER 이상 (MAINTENANCE 또는 ADMIN, EXECUTIVE 읽기 전용 선택) | 문의 상세, 첨부 상태                       | 상태 변경, 답변 등록     | MVP      |
...
| 기능 영역 | PUBLIC_USER | TEAM_MEMBER | TEAM_LEAD | DEPT_HEAD | EXECUTIVE | ADMIN |
| --- | --- | --- | --- | --- | --- | --- |
| 공개 뉴스 / 공지 / 자료실 조회 | 가능 | - | - | - | - | - |
| 공개 문의 등록 / 지원서 제출 | 가능(로그인 필요) | - | - | - | - | - |
| 마이페이지 / 내 지원 내역 | 가능 | - | - | - | - | - |
| 내부 공지 / 사원 디렉토리 | - | 가능 | 가능 | 가능 | 가능 | 가능 |
| 전자결재 | - | 가능 | 가능 | 가능 | 가능 | 가능 |
| 지원자 검토 | - | HR 소속 시 가능 | HR 소속 시 가능 | HR 소속 시 가능 | 읽기 전용(선택) | 가능 |
| 고객 문의 대응 | - | MAINTENANCE 소속 시 가능 | MAINTENANCE 소속 시 가능 | MAINTENANCE 소속 시 가능 | 읽기 전용(선택) | 가능 |
| 공개 사용자 관리 | - | 불가 | 불가 | 불가 | 불가 | 가능 |
| 파일 보안 이벤트 로그 확인 | - | 불가 | 불가 | 불가 | 불가 | 가능 |
| 감사 로그 / 운영 관리 | - | 불가 | 불가 | 불가 | 제한적 | 가능 |

내부 권한 세분화(직급 `ADMIN/EXECUTIVE/DEPT_HEAD/TEAM_LEAD/TEAM_MEMBER`, 부서별 접근, 비밀번호 강제 변경 규칙) 정본은
[docs/rules/INTERNAL_ACCESS_CONTROL_POLICY.md](../rules/INTERNAL_ACCESS_CONTROL_POLICY.md)를 따른다.
