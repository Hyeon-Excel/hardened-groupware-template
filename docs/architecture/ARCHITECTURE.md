...
### 3.1 DMZ는 프록시 역할만 수행한다 -> 프록시여 리버스 프록시여 (클대신 요청하는 서버냐 서버대신 요청하는 서버냐)
...
- Idle timeout:
  - 공개 사용자 세션: 30분 -> 세션 권장시간은 내외부 모두 600초가 권장입니다 하기 타임아웃 포함 길게 잡으신 이유가 있으실까요? / 취약점 유발 목적으로 만드신건가
  - 내부 사용자 세션: 15분
- Absolute timeout:
  - 공개 사용자 세션: 12시간
  - 내부 사용자 세션: 8시간
...
- 쿠키 속성:
  - `HttpOnly` -> 외부 자바스크립트 요청을 받지 않겠다는 건데, 보안 설정후 어디까지 검증하고 보고서화 할 것인지
  - `Secure` (HTTPS 환경) -> 인증서는 실인증서 말고 가상으로 할 것인지 (확정 요)
...
- 내부망 또는 내부 Docker 네트워크를 통한 호출 -> 내부내부 끼리의 네트웍을 의미하는 것?(backend → external-api (컨테이너 이름으로 호출)?)
