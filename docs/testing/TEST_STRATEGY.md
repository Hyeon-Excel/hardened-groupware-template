...
- 소유권 불일치(IDOR) 응답은 `403` 또는 `404`가 반환될 수 있음을 허용하고, 실제 동작을 기록한다.
...
- 재동기화 잡 정합성 검증
- 소유권 불일치(IDOR) 응답을 `404`로 고정해 리소스 존재 여부 노출이 없는지 검증
